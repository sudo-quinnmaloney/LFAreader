...
graphs = True  # Graph pixel intensity along the strip (spike == activated strip)
...
def getData(imagePath, drawLines, graphPeaks):
    ...
    if center == 0:
        ...
        center = bound - defaultCenterSpace if (np.mean(x[int(y_x / 2)][bound - defaultCenterSpace]) > np.mean(
            x[int(y_x / 2)][bound + defaultCenterSpace])) else bound + defaultCenterSpace
    ...
    widths, width_heights, left_ips, right_ips = peak_widths(lateralSum, peaks, rel_height=0.5, prominence_data=None,
                                                             wlen=None)
    ...
    for i in range(len(peaks)):
        ...
        values.append(np.sum(np.sum(
            np.sum(rotatedImg[(peak - int(y_range / 2)):(peak + int(y_range / 2)), crop_left:crop_right, :], axis=2),
            axis=1)))
    ...
    while (len(values) > 2):
        ...
        left_ips = np.append(left_ips[:min_index], left_ips[min_index + 1:]) if min_index < len(
            left_ips) - 1 else left_ips[:-1]
        right_ips = np.append(right_ips[:min_index], right_ips[min_index + 1:]) if min_index < len(
            right_ips) - 1 else right_ips[:-1]

    # find reference data points
    values.append(np.sum(np.sum(np.sum(
        rotatedImg[(referenceDistance1 - int(y_range / 2)):(referenceDistance1 + int(y_range / 2)),
        crop_left:crop_right, :], axis=2), axis=1)))
    values.append(np.sum(np.sum(np.sum(
        rotatedImg[(referenceDistance2 - int(y_range / 2)):(referenceDistance2 + int(y_range / 2)),
        crop_left:crop_right, :], axis=2), axis=1)))
    ...
    for i in range(len(peaks)):
        ...
        peak = int((left_ips[i] + right_ips[i]) / 2)
        rotatedImg = cv2.rectangle(rotatedImg, (crop_left, peak + int(y_range / 2)),
                                   (crop_right, peak - int(y_range / 2)), (255, 100, 200), 10)

    # visualize reference points as well
    rotatedImg = cv2.rectangle(rotatedImg, (crop_left, referenceDistance1 + int(y_range / 2)),
                               (crop_right, referenceDistance1 - int(y_range / 2)), (255, 100, 200), 10)
    rotatedImg = cv2.rectangle(rotatedImg, (crop_left, referenceDistance2 + int(y_range / 2)),
                               (crop_right, referenceDistance2 - int(y_range / 2)), (255, 100, 200), 10)
    ...
    rotatedImg = cv2.putText(rotatedImg, 'Ref 1', (crop_right, referenceDistance1 - int(y_range / 2)),
                             cv2.FONT_HERSHEY_SIMPLEX,
                             3, (255, 100, 200), 2, cv2.LINE_AA)
    rotatedImg = cv2.putText(rotatedImg, 'Ref 2', (crop_right, referenceDistance2 - int(y_range / 2)),
                             cv2.FONT_HERSHEY_SIMPLEX,
                             3, (255, 100, 200), 2, cv2.LINE_AA)
